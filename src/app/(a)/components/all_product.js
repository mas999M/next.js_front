...
export default function AllProduct() {
    ...
    return (
        <>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 container mx-auto p-6 md:p-10">

                {allProducts.map((product, index) => {
                    return (
                        <div key={index} className="w-full sm:w-80 border border-blue-200 rounded-lg shadow-md p-4">
                ...
                <span className="absolute top-2 left-2 bg-orange-400 text-white text-xs font-semibold px-2 py-1 rounded-full">
                  -20%
                                    ...
                                    <div>
                                        <img src={product.image_url} alt="Product Image" className="object-contain w-full h-[270px]" />
                                    </div>
                                </div>
                            </Link>

                            <div className="mt-4">
                                <h3 className="text-gray-800 font-medium text-base">{product.name}</h3>
                                <p className="uppercase text-green-600 text-xs font-medium">Already</p>
                                <div className="flex space-x-1 text-orange-500 text-sm mt-1">
                                    {[...Array(4)].map((_, idx) => (
                                        <svg key={idx} xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="currentColor" viewBox="0 0 20 20">
                                ...
                                <div className="flex items-end justify-between">
                                    ...
                                    <form onSubmit={handleSubmit}>
                                        ...
                                        <button type="submit" className="w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center shadow text-white">
